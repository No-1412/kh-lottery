				...
				success: function(response) {
					...
					if(response.root){
						...
						if(response.data.records && response.data.records > 0 && response.data.root && response.data.root.length > 0) {
							...
							opts.success(response.data.root);
						} else {
							opts.success([]);
							opts.emptyData();
						}
...
//			infos.append(infosHtml);
			...
			if(pageCount > 2){
				...
				for (var i = startPage; i <= endPage; i++) {
					var thisPage = $('<li>').html($('<a class="page" href="javascript:;">').html(i));
					if(i == page) {
