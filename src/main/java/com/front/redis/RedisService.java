...
public class RedisService {
	...
	private static CmsRedisFactory _instance = CmsRedisFactory.getInstance();

	private static Logger logger = LoggerFactory.getLogger(RedisService.class);

	private static int default_expire = 300;

	private CmsJedisPool redisPool;

	private FstSerializer fstSerialize = FstSerializer.getInstance();

	private RedisService(CmsJedisPool redisPool) {
	 ...
	 * @return
	 */
	public static RedisService newRedisService(CmsJedisPool redisPool) {
	...
	public String flushDB(int dbIndex) {
		...
		return executor.execute("flushDB", String.valueOf(dbIndex));
	}
	...
	public String mset(String... keys) {
		...
		return executor.execute("mset", StringUtils.join(keys));
	}
	...
	public Long lpush(String key, String... val) {
		...
		return executor.execute("lpush", key, StringUtils.join(val));
	}
	...
	public Long sadd(String key, int expire, String... val) {
		...
		return executor.execute("sadd", key, StringUtils.join(val));
	}
	...
	public Long srem(String key, String... val) {
		...
		return executor.execute("srem", key, StringUtils.join(val));
	}
	...
	public Long zrem(String key, String... members) {
		...
		return executor.execute("zrem", key, StringUtils.join(members));
	}
	...
	public Long hincrBy(String key, String field, Long value) {
		...
		return executor.execute("hincrBy", key, field, StringUtils.obj2String(value));
	}
	...
	public Long hdel(String key, String... fields) {
		...
		return executor.execute("hdel", key, StringUtils.join(fields));
	}
	...
	public Long del(int dbIndex, byte[]... keys) {
		...
		return executor.execute("del", StringUtils.join(keys));
	}
	...
	public void subscribe(JedisPubSub sub, String... channels) {
		...
		executor.execute("subscribe", StringUtils.join(channels));
	}
	...
	public void psubscribe(JedisPubSub sub, String... channels) {
		...
		executor.execute("psubscribe", StringUtils.join(channels));
	}
	...
	public Long publish(String channel, Object message) {
		RedisExecutor<Long> executor = new RedisExecutor<Long>() {
			@Override
			Long action(Jedis redis) {
				return redis.publish(StringUtils.getString2Utf8(channel), fstSerialize.serialize(message));
			}
		};
	...
	private byte[] getString2Utf8(Object obj) {
		return StringUtils.getString2Utf8(obj);
	}
	...
	private abstract class RedisExecutor<T> {
		...
		public T execute(String operation, String... param) {
			...
			try {
				...
				result = action(redis);
				String content = "param=" + StringUtils.join(param) + ", operation=" + operation;
				remark.append(content);

				logger.info("RedisExecutor.remark:{},result:{};content:" + content, remark, result);
